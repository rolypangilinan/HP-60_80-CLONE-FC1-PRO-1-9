...
class DatabaseManager:
    ...
    def insert_record(self, kitting_no, lineout_reason, elapsed_time, pass_ng, process_no):
        ...
        try:
            ...
            # Adjust kitting_no: web shows NEXT kitting, but we need to store ACTUAL processed kitting
            # Subtract 1 if kitting_no is a valid number greater than 0
            if kitting_no is not None and kitting_no != '':
                try:
                    kitting_num = int(kitting_no)
                    if kitting_num > 0:
                        kitting_no = str(kitting_num - 1)
                    else:
                        kitting_no = '0'
                except ValueError:
