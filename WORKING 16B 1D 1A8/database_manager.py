...
class DatabaseManager:
    ...
    def connect(self):
        ...
        try:
            ...
            # Create out_reasons table for operator OUT reasons (shared across all processes)
            create_out_reasons_table = """
            CREATE TABLE IF NOT EXISTS out_reasons (
                id INT AUTO_INCREMENT PRIMARY KEY,
                reason VARCHAR(255) NOT NULL UNIQUE,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP
            )
            """
            cursor.execute(create_out_reasons_table)
            ...
            for reason in default_out_reasons:
                cursor.execute("INSERT IGNORE INTO out_reasons (reason) VALUES (%s)", (reason,))
    ...
    def get_out_reasons(self):
        ...
        try:
            connection = self.get_connection()
            cursor = connection.cursor(dictionary=True)
            cursor.execute("SELECT id, reason FROM out_reasons ORDER BY id")
    ...
    def reset_records_only(self):
        ...
        try:
            ...
            print(f"Auto-reset: Deleted {deleted} process records (manpower preserved)")
            return True
        except Error as e:
